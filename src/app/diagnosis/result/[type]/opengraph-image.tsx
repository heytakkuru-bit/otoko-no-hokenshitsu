...
export default async function Image({ params }: { params: { type: string } }) {
  ...
  const fonts = [
    { name: 'Noto Sans JP', data: fontBold, weight: 700 as const, style: 'normal' as const },
    { name: 'Noto Sans JP', data: fontRegular, weight: 400 as const, style: 'normal' as const },
  ];

  // タイプが見つからない場合のフォールバック
  if (!typeData) {
    return new ImageResponse(
      (
        ...
        >
          <span
            style={{ color: '#C8A96E', fontSize: 56, fontFamily: 'Noto Sans JP', fontWeight: 700 }}
          >
            {LABEL}
          </span>
        </div>
  ...
  const charImageUrl = `${BASE_URL}/characters/${typeData.characterImageFile}`;
